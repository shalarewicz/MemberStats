...
# 	5. TODO:Error handling to prevent data corruption
...
"""
	...
	Read member stats from member info tab
		...
		assert members contains the member
			throw a detailed exception if not
...
#####################################################################################################################
# Imports for Client Authentication
...
SPREADSHEET_ID = '1q1z_g1oMbG25fO31wEltBHXwJpQYz2DoRwUF_twRSGw' #TODO Change to correct sheet
RETENTION_SHEET = '0' #TODO REMOVE IF NOT USED 
#COLOR_SHEET = '1177833353' TODO Remove if not used
...
members = {}
memberInfo = recordtype('memberInfo', 'lastContact phone stats row')
...
for row in memberStats:
	if onHeader:
		...
		members[mem] = memberInfo(date, phone, stats, rowNum)
		rowNum += 1
...
#TODO Fix the way this reads a text file
...
enrollmentCallRange = 'Current!J3:M3'#TODO Named range to allow for addition of new employees
...
def addMemberStats(mem):
	result = []
	result.append(mem.lastContact)
	result.append(mem.phone)
	for stat in mem.stats:
		result.append(int(stat))
	return result
	
for mem in members:
	newMemberStats[members[mem].row] = addMemberStats(members[mem])
...
memberInfoUpdateRange = MEMBER_STATS_SHEET + '!B2:' + getA1ColumnNotation(NUMBER_OF_MEMBER_STATS + 5)
...
updateRange = 'Support Outreach Administrators!E2:F'
...
email = email + "Pings (includes New Organizations; does not include Sales Pings): " + pingsLessSales + "\n"
